...
namespace Ion.CognitiveServices
{
    public static class Resolver
    {
        ...
        {
            if (Resolver.LiteralTypeMap.ContainsKey(tokenType))
            {
                return Resolver.LiteralTypeMap[tokenType]();
            }
        ...
        {
            return Resolver.PrimitiveType(token.Type);
        }

        public static LLVMValueRef Literal(Token token, PrimitiveType type)
        {
            return Resolver.Literal(token.Type, token.Value, type);
        }
